...
**Relative abundance** - *proportion of reads recruited to a split out of the total reads recruited to that split across all samples*
...
**Max-normalized ratio** - *proportion of reads recruited to a split out of the maximum number of reads recruited to that split in any sample*
...
**Mean overage Q2Q3** - *average depth of coverage across split excluding nucleotide positions with coverage values falling outside of the interquartile range for that split*
...
**Coverage STD** - *the standard deviation of coverage values for a given split*
...
**Detection** - *the proportion of that split that is covered at least 1X*
...
**Variability** -	*single-nucleotide variants per kilobase*
